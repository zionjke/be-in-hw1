export const destruct = (e: unknown, inLevel: number = 7) => {
    return transformToSafeStringify(e, inLevel)
}
...
export const transformToSafeStringify = (some: unknown, inLevel: number): any => {
    if (typeof some !== 'object') {
        ...
        if (Array.isArray(some)) {
            if (!inLevel) {
                ...
                return some.map(x => transformToSafeStringify(x, inLevel - 1))
            }
        } else {
            if (!inLevel) {
                ...
                for (const y in newSome) {
                    x[y] = transformToSafeStringify(newSome[y], inLevel - 1)
                    i++
                }
